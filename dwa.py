                ...
                for i in check:
                    if i[0] == L and i[1] == password:
                        ...
                        def insert():
                            ...
                            A=Entry(K,width=30,bd=3)
                            A.place(x=200,y=87)
                            Label(K,text="Your new item price?",font="Verdana 15").place(x=20,y=140)
                            V=Entry(K,width=30,bd=3)
                            V.place(x=245,y=146)
                            def Insert():
                                Kfc_Menu_Entry= A.get()
                                Kfc_Menu_Price_Entry= V.get()
                                if (Kfc_Menu_Entry == "" or Kfc_Menu_Price_Entry == ""):
                                    Messagebox.showinfo("Alert","Please enter what do you want to insert in your menu")
                                else:
                                    con=mysql.connect(host="localhost",user="root",database="online_order")
                                    cursor=con.cursor()
                                    cursor=con.cursor()
                                    cursor.execute("insert into kfc values('" + Name + "','" + Price + "')")
                                    cursor.execute("commit")
                                    Messagebox.showinfo("Status","Your new item has been succesfully inserted into you menu")
                                    con.close()
